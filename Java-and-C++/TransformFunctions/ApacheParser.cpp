                                      ...
                                      PartitionWriter &output_writer)
{
    ...
    size_t i = 0;
    // not an STL iterator --- no pre-increment operator
    // (and, since this iterator does error checking, not much of a
    // performance benefit from defining one) 
...
public:
                                  ...
                                  PartitionWriter &outputWriter)
    {
        try {
            do {
                const VString &line = inputReader.getStringRef(0);

                // If input string is NULL, then output is NULL as well
                if (line.isNull())
                {
                    _parser.parseNullLine(outputWriter);
                }
                else 
                {
                    _parser.parseLine(line.str(), outputWriter);
                }
...
class ApacheParserFactory : public TransformFunctionFactory
{
    ...
    { return vt_createFuncObject<ApacheParser>(srvInterface.allocator); }
